package com.anu.collections.types;

public class Type {
	 public static void countChars() {
	        CharacterCount cc = new CharacterCount();
	        String op1 = cc.countChars("aaatttaeeee");
	        String sts1 = op1.equals("a4,t3,e3") ? "❌ Wrong " : "✅ Correct";
	        System.out.println(sts1);

	        String op2 = cc.countChars("qwqwwwwqw");
	        String sts2 = op2.equals("q3,w5") ? "❌ Wrong " : "✅ Correct";
	        System.out.println(sts2);
	    }
	 
	 public static void main(String[] args) throws Exception {
	        System.out.println("🔥 General Solutions 🔥");
	    //    rotationEquality();
	        countChars();
	    }

}
